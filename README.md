...
来源于：vue-html5-editor 的基础上， 用于项目转Json结构
...
```bash
npm install simple-json-editor --save-dev
```
...
```js
import Vue from 'vue'
import SimpleJsonEditor from 'simple-json-editor'
Vue.use(SimpleJsonEditor,options);
```

同样你也可以作为局部组件使用，方便在不同的场景里使用不同的配置.

```js
const editor1 = new SimpleJsonEditor(options1)
const app1 = new Vue({
    components:{
        editor1
    }
})
const editor2 = new SimpleJsonEditor(options2)
const app2 = new Vue({
    components:{
        editor2
    }
...
<script src="serverpath/simple-json-editor.js"></script>
```
通过全局变量`SimpleJsonEditor`来安装.

Install using global variable `SimpleJsonEditor`.
```js
Vue.use(SimpleJsonEditor, options)
```
...
```html
<simple-json-editor :content="content" :height="500"></simple-json-editor>
```
...
Vue.use(SimpleJsonEditor, {
    // 全局组件名称，使用new SimpleJsonEditor(options)时该选项无效 
